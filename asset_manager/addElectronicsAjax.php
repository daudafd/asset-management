...
//to set location variable as the unique variable for all input field.
    ...
    $scan_code = "ODIRS-EE-".date("Y")."-".time();
    $ee_sub_category = htmlentities(mysqli_real_escape_string($conn, $_POST['ee_sub_category']));
    ...
    $sql = "INSERT INTO tbl_electronics (scan_code, ee_sub_category, name, serial_no, model, brand, location, own_degree, status, asset_condition, initial_value,
    procure_date, deprec_value, address, size, quantity, dept, descriptn, created_date) VALUES ('$scan_code', '$ee_sub_category', '$name', '$serial_no',
    ...
    procure_date, deprec_value, address, size, quantity, dept, description, created_date) VALUES ('$scan_code', '$ee_sub_category', '$name', '$serial_no',
    '$model', '$brand', '$location',  '$own_degree',  '$status', '$asset_condition', '$initial_value', '$procure_date', '$deprec_value', '$address',
    '$size', '$quantity', '$dept', '$descriptn', '$created_date')";
