...
//$conn = mysqli_connect("localhost", "odirscom_AssetMGT", "ODIRSAssetMgt_2021", "odirscom_asset") or die("Sorry, cannot connect to the database");
//$conn = mysqli_connect("localhost", "odirsorg_odirsasset", "Asset@ODIRS2021", "odirsorg_asset") or die("Sorry, cannot connect to the database");
...
function fetchAssetLimit($tblname, $tblId){
    global $conn;
    $sql = "SELECT allasset.scan_code as scanCode, allasset.name as assetName, allasset.location as assetLocation, allasset.status as assetStatus, attachments.attachment as assetImage FROM $tblname JOIN attachments on allasset.scan_code = attachments.application_number ORDER BY $tblId DESC LIMIT 5";
    $qry = mysqli_query($conn,$sql);
    return $qry;
}

function getLocationCount($tableName, $location){
    global $conn;
    $sql = "SELECT * FROM $tableName WHERE location LIKE '%$location%'";
...
function getDept($tableName, $dept){
    global $conn;
    $sql = "SELECT * FROM $tableName WHERE dept = '$dept'";
    $query = mysqli_query($conn, $sql);
    return $query;
}
