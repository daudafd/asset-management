...
//to set location variable as the unique variable for all input field.
    ...
    $scan_code = "ODIRS-OI-".date("Y")."-".time();
    //$cdate = date('Y-m-d h:i:s');
    $mdate = date('Y-m-d h:i:s');
    
    $name = htmlentities(mysqli_real_escape_string($conn, $_POST['name']));
    $brand = htmlentities(mysqli_real_escape_string($conn, $_POST['brand']));
    $oi_sub_category = htmlentities(mysqli_real_escape_string($conn, $_POST['oi_sub_category']));
    ...
    $sql = "INSERT INTO tbl_otheritems (scan_code, name, brand, oi_sub_category, location, own_degree, status, asset_condition, dept, descriptn, created_date)
    VALUES ('$scan_code', '$name', '$brand', '$oi_sub_category', '$location',  '$own_degree',  '$status', '$asset_condition', '$dept', '$descriptn', '$created_date')";
    $qry = mysqli_query($conn, $sql);

    $sql2 = "INSERT INTO allasset (scan_code, name, brand, sub_category, location, own_degree, status, asset_condition, dept, description, created_date)
    VALUES ('$scan_code', '$name', '$brand', '$oi_sub_category', '$location',  '$own_degree',  '$status', '$asset_condition', '$dept', '$descriptn', '$created_date')";
